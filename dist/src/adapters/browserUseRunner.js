...
export async function executeBrowserUse(job, options = {}) {
    ...
    try {
        ...
        return createDryRun(job, outputRoot, `Browser-Use request failed: ${error.message}`);
    }
}
async function readSession(explicitPath) {
...
async function createDryRun(job, outputRoot, reason) {
    const targetDir = await prepareOutputDirectory(outputRoot, job.scenario);
    await writeTextFile(targetDir, "instructions.txt", `Scenario: ${job.scenario}\n\nInstructions:\n${job.instructions}\n\nReason: ${reason}\n`);
    return {
        scenario: job.scenario,
        screenshotPath: "",
        htmlPath: "",
...
async function prepareOutputDirectory(root, scenario) {
...
function toRelativePath(target) {
