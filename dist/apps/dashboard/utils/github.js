export async function queueTaskViaGitHub(payload, options) {
    ...
    const commitResponse = await fetch(`https://api.github.com/repos/${owner}/${repo}/contents/${encodePath(path)}`, {
        method: "PUT",
        headers,
        body: JSON.stringify({
    ...
    if (!commitResponse.ok) {
        ...
        throw new Error(`GitHub queue commit failed (${commitResponse.status}): ${truncate(errorText, 240)}`);
    }
    const commitJson = (await commitResponse.json());
    let workflowDispatched = false;
    if (workflow) {
        const dispatchResponse = await fetch(`https://api.github.com/repos/${owner}/${repo}/actions/workflows/${workflow}/dispatches`, {
            method: "POST",
            headers,
...
class QueuePathResolver {
    ...
    taskId() {
        const explicit = (typeof this.payload.task_id === "string" && this.payload.task_id) ||
