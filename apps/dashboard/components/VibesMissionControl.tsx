// 🔒 AUTO-GUARD: Vibeflow Mission Control (full) — mock↔live ready
...
function pct(done = 0, total = 0) { return total ? clamp(Math.round((done / total) * 100)) : 0; }
...
export type Task = {
  id?: string;
  name?: string;
  status?: 'pending' | 'in progress' | 'received' | 'testing' | 'approved' | 'done';
  tokens?: number;
  locked?: boolean;
  prompt?: string;
  dependencies?: string[];
...
function normalizeAgent(input: unknown): Agent | null {
  ...
  const id = (typeof input.id === 'string' ? input.id : undefined) ?? Math.random().toString(36).slice(2);
  const name = typeof input.name === 'string' ? input.name : 'Agent';
  const logo = typeof input.logo === 'string' ? input.logo : (typeof input.logo_url === 'string' ? (input.logo_url as string) : ICON.default);
  const tier = (input.tier === 'W' || input.tier === 'M' || input.tier === 'Q') ? input.tier : 'M';
  const status = typeof input.status === 'string' ? input.status.replace('_', ' ') : 'idle';
  const tokens = typeof input.tokens === 'number' ? input.tokens : 0;
  const warnings = isArray(input.warnings) ? input.warnings.filter(w => typeof w === 'string') as string[] : [];
  return { id, name, logo, tier, status, tokens, warnings };
}
...
function normalizeTask(input: unknown): Task | null {
  ...
  const t: Task = {
    ...
    status: ((): Task['status'] => {
      ...
      const allowed = ['pending', 'in progress', 'received', 'testing', 'approved', 'done'] as const;
      return typeof s === 'string' && (allowed as readonly string[]).includes(s) ? (s as Task['status']) : 'pending';
    })(),
    tokens: typeof input.tokens === 'number' ? input.tokens : 0,
    locked: typeof input.locked === 'boolean' ? input.locked : false,
    prompt: typeof input.prompt === 'string' ? input.prompt : undefined,
    dependencies: isArray(input.dependencies) ? input.dependencies.filter(x => typeof x === 'string') as string[] : [],
    code_url: typeof input.code_url === 'string' ? input.code_url : undefined,
  };
  return t;
}

function normalizeSlice(input: unknown): Slice | null {
  if (!isRecord(input)) return null;
  const id = (typeof input.id === 'string' ? input.id : undefined) ?? Math.random().toString(36).slice(2);
  const name = typeof input.name === 'string' ? input.name : 'Slice';
  const tasksDone = typeof input.tasksDone === 'number' ? input.tasksDone : 0;
  const tasksTotal = typeof input.tasksTotal === 'number' ? input.tasksTotal : 0;
  const tokens = typeof input.tokens === 'number' ? input.tokens : 0;
  const tasksRaw = input.tasks;
  const tasks: Task[] = isArray(tasksRaw)
    ? (tasksRaw.map(normalizeTask).filter(Boolean) as Task[])
    : [];
  return { id, name, tasksDone, tasksTotal, tokens, tasks };
}
...
/* ----------------------------- Sidebars ---------------------------- */
const SliceDock: React.FC<{
  slices: Slice[];
  onSliceClick?: (s: Slice) => void;
  onDocs?: () => void;
  onLogs?: () => void;
}> = ({ slices, onSliceClick, onDocs, onLogs }) => (
...
const AgentHangar: React.FC<{
  agents: Agent[];
  onAgentClick?: (a: Agent) => void;
  onAdd?: () => void;
  onViewAll?: () => void;
}> = ({ agents, onAgentClick, onAdd, onViewAll }) => (
    ...
    {agents.map((a) => (
      <div key={a.id} onClick={() => onAgentClick?.(a)} className="flex flex-col items-center cursor-pointer hover:opacity-90 active:scale-[0.98] transition relative">
        <div className="relative">
          <img
            src={a.logo || ICON.default}
            alt={a.name}
            className="h-10 w-10 rounded-full border border-slate-700 object-contain bg-slate-800"
            loading="lazy"
            decoding="async"
            onError={(e) => {
...
/* --------------------------- Center Orbit Hub ---------------------- */
const OrbitSliceHub: React.FC<{
  slice: Slice;
  agents: Agent[];
  onAgentClick?: (a: Agent) => void;
  onSliceClick?: (s: Slice) => void;
}> = ({ slice, agents, onAgentClick, onSliceClick }) => {
  ...
  return (
    <div className="relative w-36 h-40 md:w-44 md:h-48 xl:w-48 xl:h-52 flex flex-col items-center justify-center mt-6">
      ...
      {agents.map((agent, i) => {
        const angle = (i * (360 / Math.max(agents.length, 1))) - 90;
        const rad = (angle * Math.PI) / 180;
        const innerR = 26; const outerR = 50;
        const x1 = 50 + innerR * Math.cos(rad); const y1 = 50 + innerR * Math.sin(rad);
        const x2 = 50 + outerR * Math.cos(rad); const y2 = 50 + outerR * Math.sin(rad);
        const labelX = x2 + Math.cos(rad) * 10; const baseLabelY = y2 + Math.sin(rad) * 12;
        const labelYOffset = Math.sin(rad) < -0.2 ? -4 : 0; const labelY = baseLabelY + labelYOffset;
          ...
          <svg key={agent.id} viewBox="0 0 100 100" className="absolute inset-0 overflow-visible cursor-pointer" onClick={() => onAgentClick?.(agent)}>
            ...
            <foreignObject x={x2 - 8} y={y2 - 8} width="16" height="16">
              <div className="relative flex flex-col items-center">
                <img
                  src={agent.logo || ICON.default}
                  alt={agent.name}
                  width="16"
                  height="16"
                  className="rounded-full border border-slate-700 bg-slate-800"
                  loading="lazy"
                  decoding="async"
                  onError={(e) => {
                    const img = e.currentTarget as HTMLImageElement & { dataset: Record<string, string> };
                    if (img.dataset.fallbackApplied) return; img.src = ICON.default; img.dataset.fallbackApplied = '1';
                  }}
                />
...
function HeaderBar({ onROI }: { onROI: () => void }) {
  return (
    <div className="sticky top-0 z-10 bg-[#0a0d17] pt-1 pb-1 -mx-10 px-10">
      ...
      <p className="text-[11px] text-slate-400 max-w-2xl mb-2">
        Vibeflow is an AI-driven multi-agent orchestration framework that plans, routes, executes, and validates tasks across systems with full observability and ROI tracking.
      </p>
    </div>
  );
}
...
export default function VibesMissionControl() {
  // global styles + gentle pulse
  useEffect(() => {
    const st = document.createElement('style');
    st.innerHTML = scrollbarStyles + `
      @keyframes goldPulse { 0% { box-shadow: 0 0 0 0 rgba(234,179,8,0.45); } 70% { box-shadow: 0 0 0 14px rgba(234,179,8,0); } 100% { box-shadow: 0 0 0 0 rgba(234,179,8,0); } }
    `;
    document.head.appendChild(st);
    return () => { document.head.removeChild(st); };
  }, []);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const j: SnapshotWire = await res.json();
        if (cancelled) return;

        const agentsWire = isArray(j.agents) ? j.agents : [];
        const slicesWire = isArray(j.slices) ? j.slices : [];

        const a = agentsWire.map(normalizeAgent).filter(Boolean) as Agent[];
        const s = slicesWire.map(normalizeSlice).filter(Boolean) as Slice[];

        setAgents(a.length ? a : demoAgents);
        setSlices(s.length ? s : demoSlices);
      } catch {
        setAgents(demoAgents);
        setSlices(demoSlices);
      }
    })();
  ...
  const totalTokens = useMemo(
    () => agents.reduce((acc, x) => acc + (x.tokens || 0), 0) + slices.reduce((acc, x) => acc + (x.tokens || 0), 0),
    [agents, slices]
  );
      ...
      <main className="flex-1 h-full min-h-0 px-10 pt-0 ml-24 mr-24 overflow-y-auto">
        ...
        <div className="relative w-full max-w-2xl h-2 rounded-full bg-slate-800 mb-2">
          <div
            ...
            style={{ width: `${clamp(Math.round((activeSlices.length / Math.max(slices.length, 1)) * 100))}%` }}
          />
        </div>
