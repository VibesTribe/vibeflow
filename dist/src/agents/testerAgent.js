...
export class TesterAgent {
    async execute(input) {
        ...
        if (input.packet.metadata?.scenario) {
            results.push(await this.invokeRunner("run_visual_tests", {
                scenario: input.packet.metadata?.scenario,
            }));
        }
        return reduceOutcomes(results);
    }
    invokeRunner(skillId, payload) {
        ...
        return new Promise((resolve) => {
            ...
            child.on("close", (code) => {
                ...
                try {
                    ...
                    resolve({
                        status: "error",
                        reason: `Skill ${skillId} returned invalid JSON: ${error.message}`,
                    });
                }
            });
        });
...
function reduceOutcomes(outcomes) {
