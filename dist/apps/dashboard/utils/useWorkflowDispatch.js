...
function sanitizeInputs(record = {}) {
    const next = {};
...
export function useWorkflowDispatch(repo = defaultRepo, token = defaultToken) {
    const [status, setStatus] = useState("idle");
    const [error, setError] = useState(null);
    const available = useMemo(() => Boolean(repo && token), [repo, token]);
    const trigger = useCallback(async ({ workflow, inputs, ref }) => {
        if (!available) {
