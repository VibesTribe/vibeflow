...
const DOC_LINKS = [
  { label: "VibePilot Code Graph (Graphify)", path: "https://graphs.vibestribe.rocks/graph/vibepilot-graph.html", external: true },
  { label: "VibePilot AST Graph (code-review-graph)", path: "https://graphs.vibestribe.rocks/graph/vibepilot-code-review-graph.html", external: true },
...
const PENDING_STATUSES = new Set<TaskStatus>(["pending", "failed"]);
                ...
                return (
                  <li key={sub.model_id} className="roi-panel__subscription-item">
                    ...
                    <div className="roi-panel__subscription-stats" style={{ marginTop: "4px" }}>
                      ...
                      <span style={{ color: "#ffffff" }}>{(sub.tokens_used > 0 && sub.subscription_cost_usd && sub.subscription_cost_usd > 0) ? (() => { const tp = sub.tokens_used / sub.subscription_cost_usd; return tp >= 1000000 ? `${(tp/1000000).toFixed(1)}M` : tp >= 1000 ? `${(tp/1000).toFixed(1)}K` : tp.toFixed(0); })() + " tokens per $1" : ""}</span>
                    </div>
                  </li>
                );
              })}
